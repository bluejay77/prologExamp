...
% Chapter 9 example
% input and grammar for example 2, "John called Mary from Denver",
% section 9.2.3
...
lex_rule(noun, ['helsinki']).
lex_rule(noun, ['rovaniemi']).
...
lex_rule(det_time, ['today']).
lex_rule(det_time, ['yesterday']).

% and this is there to demonstrate the predicates with the Prolog
% written Earley parser:


go :- go(s).                   % parse input as a sentence
